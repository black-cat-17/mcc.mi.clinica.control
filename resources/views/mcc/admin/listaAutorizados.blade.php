<div class="px-5 mt-4 container-fluid">
    ...
    <div class="mb-3 d-flex justify-content-end">
        <a href="{{ route('admin.autorizados.nuevo') }}" class="btn btn-secondary">Agregar Nuevo Autorizado</a>
    </div>
            ...
            <tbody>
                ...
                @else
                    @foreach ($autorizados as $autorizado)
                        <tr>
                            <td class="text-center">{{ $autorizado->ID_autorizado }}</td>
                            <td>{{ $autorizado->paciente->nombre }} {{ $autorizado->paciente->apellidos }}</td>
                            <td class="text-center">{{ $autorizado->ID_facultativo }}</td>
                            <td class="text-center">{{ $autorizado->fecha_alta->format('d-m-Y') }}</td>
                            <td class="text-center">
                                <span>
                                    {{ $autorizado->activo == 1 ? 'Sí' : 'No' }}
                                </span>
                            </td>
                            <td class="text-center">
                                <div class="gap-2 d-flex justify-content-center">
                                    <a href="{{ route('admin.autorizados.edit', $autorizado->ID_autorizado) }}"
                                        class="btn btn-sm btn-secondary">
                                        Editar
                                    </a>
                                    <form action="{{ route('admin.autorizados.destroy', $autorizado->ID_autorizado) }}"
                                        method="POST"
    ...
    <div class="mt-3 d-flex justify-content-center">
        {{ $autorizados->links('pagination::bootstrap-5') }}
    </div>
