@extends('mcc.facultativo.layout') <!-- Asegúrate de usar el layout correcto según el tipo de usuario -->
        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="mb-3 card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('facultativo.autorizados.update', $autorizado->ID_autorizado) }}"
                            method="POST">
                            @csrf
                            @method('PUT')
                            {{-- Nombre del paciente --}}
                            <div class="mb-3">
                                ...
                                <select name="ID_user" id="ID_user" class="form-control" disabled>
                                    @foreach ($pacientes as $paciente)
                                        <option value="{{ $paciente->id }}"
                                            @if ($autorizado->ID_user == $paciente->id) selected @endif>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="text" class="form-control"
                                        value="{{ $autorizado->facultativo->user->nombre }} {{ $autorizado->facultativo->user->apellidos }}"
                                        readonly>
                                </div>
                                ...
                                <div class="mb-3 d-flex justify-content-between">
                                    ...
                                    <a href="{{ route('facultativo.autorizados') }}" class="btn btn-danger">Cancelar</a>
                                </div>
                        </form>
                    </div>
                </div>
